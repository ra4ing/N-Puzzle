...
/**
 * PuzzleBoard类表示NPuzzle问题的状态，继承自State类。
 * 包含了表示N-Puzzle棋盘的二维数组，以及相关的状态信息和操作方法。
 */
public class PuzzleBoard extends State {
    private static final int[][][][] zobristTable; // 用于计算zobrist
    private static final int[][][] goalTable; // 提前储存每个数字的目标位置
    public final int[][] board; // 二维数组表示拼图板上的数字
    private int[] stateCode; // 用于获取数据库位置
    private final int size; // 拼图板的大小，如3表示3x3拼图
    private int emptyRow; // 用于存储空白格的行位置
    private int emptyCol; // 用于存储空白格的列位置
    private static final double SCALE = 1.1; // 启发值的权值
    private int hash_code = -1; // 状态的zobrist哈希值
    private int heuristic = -1; // 状态的启发值
     ...
     * 构造一个新的PuzzleBoard对象。
     *
     * @param size  N-Puzzle的尺寸
     * @param board 表示N-Puzzle棋盘的二维数组
     */
...
//    }
    ...
    // 根据创建模式数据库时的哈希方式计算哈希值，用以得到该状态在模式数据库中对应的PathCost
